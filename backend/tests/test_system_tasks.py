...
class SystemTasksTest(unittest.TestCase):
    ...
    def test_task_config_is_persisted_and_can_start_sync_runner(self):
        ...
        self.addCleanup(storage.stop_system_tasks)

        initial = storage.list_system_tasks()[0]
        ...
        updated = storage.save_system_task(
            "uniportal_sync", {"enabled": True, "interval_seconds": 15}
        )
    ...
    def test_system_task_api_validates_and_applies_updates(self):
        with patch.dict(
            ...
            app = create_app()
            app.testing = True
            self.addCleanup(app.config["STORAGE"].stop_system_tasks)
